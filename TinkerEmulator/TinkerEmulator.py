...
class TinkerEmulator_Bias(object):
...
class TinkerEmulator_hmf(object):

    def __init__(self):
        self.cosmo_pars = np.load("../data/training_cosmologies.npy")
        data = np.load("../data/rotated_MF_parameters.npy")
        self.means = data[:,:,0]
        self.variances = data[:,:,1]**2
        self.rotation_matrix = np.load("../data/rotation_matrix.npy")
        self.lamb = []
        for i in range(self.variances.shape[-1]):
            self.lamb.append([])
    ...
    def _train(self):
        ...
        for i in range(N):
            k = george.kernels.ExpSquaredKernel(hps, ndim=Ncos)
            gp = george.GP(k, mean=np.mean(means[:,i]), white_noise=np.log(np.mean(stds[:, i])**2))
            gp.compute(self.cosmo_pars, stds[:,i])
    ...
    def predict_tinker_parameters(self, cosmological_parameters, redshift):
        ...
        e0,f0,g0,d1,e1,g1 = self.emulate(cosmological_parameters)
        d= d0 + x * d1
        e= e0 + x * e1
        f= f0 + x * f1
        g= g0 + x * g1
